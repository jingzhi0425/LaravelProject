    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.product.update', [$product->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bar_code_id') ? 'is-invalid' : '' }}" type="text" name="bar_code_id" id="bar_code_id" value="{{ old('bar_code_id', $product->bar_code_id) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('image_id') ? 'is-invalid' : '' }}" name="image_id" id="image_id">
                    ...
                    <option value="{{ $id }}" {{ (old('image_id') ? old('image_id') : $product->images->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
